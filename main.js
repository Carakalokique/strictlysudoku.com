...
let difficulty = document.querySelector('meta[name="sudoku-difficulty"]')?.content || 'easy'
let maxMistakes = 999
...
{
    const rows = Array(9).fill(1).map(n => [])
    let offset = 0
    for(let y = 0; y < 9; ++y)
    {
        offset += 3
...
function createCells()
{
    ...
    {
        if(!groups.columns[x])
            groups.columns[x]  = new Group()

        for(let y = 0; y < 9; ++y)
        {
            if(!groups.rows[y])
                groups.rows[y]  = new Group()

            const localID = ~~(x / 3) + '-' + ~~(y / 3)

            if(!groups.local[localID])
                groups.local[localID]  = new Group()
            ...
            if(x % 3 === 2 && x !== 8)
                cell._node.classList.add('vertical-line')

            if(y % 3 === 2 && y !== 8)
                cell._node.classList.add('horizontal-line')
        }
    }
...
function init()
{
    ...
    document.querySelectorAll('#keyboard-keys .button').forEach(elem => {
        ...
        {
            if(!Cell._selected || gameWon)
                return

            if(noteMode && id !== 0)
                Cell._selected.addNote(id)
            else
                Cell._selected.enterNumber(id)
    ...
    window.addEventListener('keydown', e => {
        ...
        {
            if(!noteMode)
                Cell._selected.enterNumber(+e.key)
            else
                Cell._selected.addNote(+e.key)
        }
    ...
    document.getElementById('mode-button-normal').addEventListener('click', () => toggleNotes(false))
    document.getElementById('mode-button-note').addEventListener('click', () => toggleNotes(true))
    document.getElementById('play-again-button').addEventListener('click', () => resetGame())
    resetGame()
    console.log(currentGroups)
    document.querySelector(`#difficulty-select option[value="${difficulty}"]`).setAttribute('selected', 'selected')
...
function numberEntered()
{
    ...
    Cell._instances.forEach(cell => {
        if(cell.value !== 0 && !cell.incorrect)
            correctCells[cell.value]++
    })
    ...
    {
        if(correctCells[i] === 9)
            buttons[i - 1].className = 'button disabled'
        else
            buttons[i - 1].className = 'button'
    }
}
...
function timeTick()
{
    ...
    document.getElementById('time-elapsed').innerText = secondsToTimeString(elapsed)
}
let gameWon = false
function checkForWin()
{
    ...
    ;[...currentGroups.rows, ...currentGroups.columns, ...currentGroups.local].forEach(group => {
        let numbers =[]
        group.forEachCell(cell => {
            if(cell.value === 0 || numbers.includes(cell.value))
                win = false
...
function winGame(win = true)
{
    ...
    setTimeout(()=>{
        if(win)
        {
            ...
            document.getElementById('popup-message').innerText = `Congrats! You solved this ${difficulty} puzzle in ${secondsToTimeString(timeElapsed2)}`
        }
        else
        {
            document.getElementById('popup').style.display = 'flex'
            document.getElementById('popup-title').innerText = 'Game Over'
            document.getElementById('popup-message').innerText = `You have made ${maxMistakes} mistakes and lost this game`
        }
    })
...
function toggleNotes(state)
{
    ...
    if(noteMode)
    {
        document.getElementById('mode-button-normal').className = 'button button-left'
        document.getElementById('mode-button-note').className = 'button button-right selected'
        document.getElementById('keyboard-keys').className = 'keys note-mode'
    }
    else
    {
        document.getElementById('mode-button-normal').className = 'button button-left selected'
        document.getElementById('mode-button-note').className = 'button button-right'
...
deferCSSImport('https://fonts.googleapis.com/css2?family=Oswald:wght@200..700&family=Roboto&family=IBM+Plex+Mono:wght@200;400;500&display=swap');
